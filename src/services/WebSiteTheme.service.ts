// backend/services/WebSiteTheme.service.ts
...
export class WebSiteThemeService {
  ...
  async getThemesByWebSiteId(websiteId: string): Promise<WebSiteThemeProps[]> {
    try {
      ...
      return await WebSiteThemeModel.find({ websiteId })
        .populate('website')
        .sort({ createdAt: -1 });
    } catch (error) {
...
return await WebSiteThemeModel.findOne({ 
  ...
  async getThemeById(themeId: string): Promise<WebSiteThemeProps | null> {
    try {
      ...
      return await WebSiteThemeModel.findById(themeId).populate('website');
    } catch (error) {
  ...
  async updateTheme(themeId: string, updateData: Partial<WebSiteThemeProps>): Promise<WebSiteThemeProps | null> {
    try {
      ...
      return await WebSiteThemeModel.findByIdAndUpdate(themeId, updateData, {
        new: true,
        runValidators: true,
      }).populate('website');
    } catch (error) {
  ...
  async updateThemeColors(
    themeId: string,
    colors: Partial<WebSiteThemeProps['colors']>
  ): Promise<WebSiteThemeProps | null> {
    try {
      if (!Types.ObjectId.isValid(themeId)) {
        throw new Error('Invalid theme ID');
      }
      ...
      if (colors.light) {
        Object.keys(colors.light).forEach((key) => {
          updateData[`colors.light.${key}`] = colors.light![key as keyof typeof colors.light];
        });
      }
      if (colors.dark) {
        Object.keys(colors.dark).forEach((key) => {
          updateData[`colors.dark.${key}`] = colors.dark![key as keyof typeof colors.dark];
        });
      }

      return await WebSiteThemeModel.findByIdAndUpdate(
        themeId,
        { $set: updateData },
  ...
  async updateThemeFonts(
    themeId: string,
    fonts: Partial<WebSiteThemeProps['fonts']>
  ): Promise<WebSiteThemeProps | null> {
    try {
      if (!Types.ObjectId.isValid(themeId)) {
        throw new Error('Invalid theme ID');
  ...
  async setActiveTheme(websiteId: string, themeId: string): Promise<WebSiteThemeProps | null> {
    try {
      ...
      await WebSiteThemeModel.updateMany({ websiteId }, { isActive: false });
  ...
  async deleteTheme(themeId: string): Promise<boolean> {
    try {
      ...
      if (theme.isActive) {
        ...
        if (otherTheme) {
          await WebSiteThemeModel.findByIdAndUpdate(otherTheme._id, { isActive: true });
        }
      }
  ...
  async getAllThemes(
    ...
    try {
      ...
      const themes = await WebSiteThemeModel.find()
        .populate('website')
        .skip(skip)
        .limit(limit)
        .sort({ createdAt: -1 });
