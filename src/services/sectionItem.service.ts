...
class SectionItemService {
  ...
  async createSectionItem(itemData: IService): Promise<IServiceDocument> {
    try {
      ...
      // Validate WebSiteId
      if (!itemData.WebSiteId || !mongoose.Types.ObjectId.isValid(itemData.WebSiteId)) {
        throw AppError.validation(`Invalid or missing WebSiteId: ${itemData.WebSiteId}`);
      }

      // Validate section (if provided)
      if (itemData.section && !mongoose.Types.ObjectId.isValid(itemData.section)) {
        throw AppError.validation(`Invalid section ID: ${itemData.section}`);
      }
      if (itemData.section) {
        const sectionExists = await SectionModel.findById(itemData.section);
        if (!sectionExists) {
          throw AppError.notFound(`Section with ID ${itemData.section} not found`);
        }
      }

      // Validate subsections (if provided)
      if (itemData.subsections && !mongoose.Types.ObjectId.isValid(itemData.subsections)) {
      ...
      // Update parent section if subsections is provided
      if (itemData.subsections) {
        await SectionModel.findByIdAndUpdate(
          itemData.subsections,
          { $addToSet: { sectionItems: sectionItem._id } },
          { session: null } // Ensure no session is used unless explicitly required
        );
      }

      return sectionItem;
    } catch (error) {
      console.error('Error in createSectionItem:', error); // Log detailed error
      if (error instanceof AppError) throw error;
      if (error.name === 'ValidationError') {
        throw AppError.validation(`Schema validation failed: ${error.message}`);
      }
  ...
  async updateSectionItemById(id: string, updateData: Partial<IService>): Promise<IServiceDocument> {
    try {
      ...
      // If subsections is being updated, check if it exists
      if (updateData.subsections && updateData.subsections.toString() !== sectionItem.subsections?.toString()) {
        const subsectionExists = await SubSectionModel.findById(updateData.subsections);
        if (!subsectionExists) {
          throw AppError.notFound(`Subsection with ID ${updateData.subsections} not found`);
        }
      ...
      if (!updatedSectionItem) {
        throw AppError.notFound(`Section item with ID ${id} not found after update`);
      }
